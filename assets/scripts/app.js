...
function buildContainerFilter() {
  ...
  const opts = ['<option value="all">All</option>'].concat(Array.from(set).sort().map(c => `<option value="${c}">${c}</option>`)).join('');
  ...
  rows.forEach(r => {
    ...
    cols.forEach(c => {
      ...
      td.addEventListener('input', () => {
        // update the underlying allRows entry: find by NO + BoxNum + ContainerNum if possible
        const allRows = files[activeKey].rows;
        ...
        if (idx === -1) {
          idx = allRows.findIndex(rr => String(rr.BoxNum ?? '') === String(r.BoxNum ?? '') && String(rr.ContainerNum ?? '') === String(r.ContainerNum ?? ''));
        }
        if (idx >= 0) {
          // update canonical field and keep extra fields intact
          files[activeKey].rows[idx][c] = td.textContent;
          // autosave to localStorage
          saveFileToLocalStorage(activeKey);
          // update UI & analytics (based on filtered rows)
          // we must recalc filtered rows with current filters
          renderFilteredAndLive();
        }
      });
...
function renderCharts(rows) {
  ...
  progressChart = new Chart(ctxDonut, {
    type: 'doughnut',
    data: { labels: ['Finished', 'Remaining'], datasets: [{ data: [totals.finished, totalRemaining] }] },
